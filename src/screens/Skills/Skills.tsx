    ...
    <div className={styles.contentWrapper}>
      <div>
        ...
        <form className={styles.form}>
          {languages.map(lang => (<div style={{display: 'flex', gap: '.5rem', alignItems: 'center'}}><TextField key={lang} value={lang} name={lang} /><DeleteIcon /></div>))}
        </form>
      </div>
      <div>
        <h3>Skills</h3>
        <form className={styles.form}>
          {skills.map(skill =>(<div style={{display: 'flex', gap: '.5rem', alignItems: 'center'}}><TextField key={skill} value={skill} name={skill} /><DeleteIcon /></div>))}
        </form>
      </div>
    </div>
